...
import { formatINR, getCurrentFY, calculateMemberNet, calculateFYTarget, formatDate } from '@/utils/financialYear';
import type { MemberDoc, TransactionDoc } from '@/types';
import { ArrowLeft, ArrowUpRight, ArrowDownRight, RotateCcw, Wallet, Calendar } from 'lucide-react';
...
const txTypeConfig = {
  ...
  opening_balance: { label: 'Opening Balance', color: 'bg-purple-500', icon: Wallet },
};
      ...
      <CardContent className="p-4">
        ...
        {tx.savingsMonth && (
          <div className="mt-2 pt-2 border-t border-border">
            <span className="text-xs bg-muted px-2 py-0.5 rounded-full">{tx.savingsMonth}</span>
          </div>
        )}
  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        ...
        >
          <Card className={cn(
            ...
            <CardContent className="p-5">
              ...
              <p className="text-3xl font-extrabold text-white mt-1">{formatINR(net)}</p>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
            <Card className="h-full">
              <CardContent className="p-4">
                ...
                <p className="text-lg font-bold mt-1">{formatINR(totalDeposit)}</p>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <Card className="h-full">
              <CardContent className="p-4">
                ...
                <p className="text-lg font-bold mt-1">{formatINR(receivable)}</p>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-1">
                ...
                <p className="text-sm font-medium">{formatINR(fyDeposited)} / {formatINR(fyTarget)}</p>
              </div>
              ...
              <div className="flex items-center justify-between mt-2">
                <p className="text-xs text-muted-foreground">{progressPct}% of target</p>
                <p className="text-xs text-muted-foreground">
                  {progressPct >= 100 ? 'Target reached!' : `${formatINR(fyTarget - fyDeposited)} remaining`}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        {/* Transaction History */}
        <section>
          <h2 className="text-lg font-semibold mb-3 px-1">Transaction History</h2>
